...
| `experiment.start({ name, command, working_dir?, wake_prompt })` | Spawn detached subprocess, return `{ uuid, status, dir }` — `dir` is the experiment's git-tracked wiki folder. See [experiments.md](experiments.md) for the full lifecycle. |
